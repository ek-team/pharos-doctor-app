<template>
	<view class="addForm">
		...
		<view class="formBG">
			...
			<view class="formName">
				<view class="formNames">
					...
					<input type="text" :disabled="id==0?false:true" v-model="form.title" placeholder="请输入表单名称..." class="nameInfo">
				</view>
			</view>
		</view>
		<view class="formAdd">
				...
				<view class="inputInfo">
					...
					<view class="inputInfos " v-if="item.type==1">
						...
						<view class="inputInfosList">
							<view class="flexA" style="margin: 28rpx 0;" v-for="(items,indexs) in item.formOptionsList" :key="indexs">
								{{items.text}}
					...
					<view class="inputInfos" v-if="item.type==3">
						...
						<view style="height: 200rpx;margin-top: 20rpx;color:#989898;border: 1px solid  #ECECEC;padding: 20rpx;">{{item.placeholder}}</view>
						<!-- 答案对应的分数 -->
						<view class="inputInfosList">
							<view class="flexA" style="margin: 28rpx 0;" v-for="(items,indexs) in item.formOptionsList" :key="indexs">
								{{items.text}}
					...
					<view class="inputInfos" v-if="item.type==2">
						...
						<view class="inputInfosList">
							<view class="flexA" style="margin: 28rpx 0;" v-for="(items,indexs) in item.formOptionsList" :key="indexs">
								<image src="/static/images/icon_cho@2x.png" mode="" class="inputInfosListImg"></image>
								{{items.text}}
								<text v-if="items.score" style="margin-left: 14rpx;">{{'('+items.score+'分)'}}</text>
							</view>
						</view>
					...
					<view class="inputInfos" v-if="item.type==6">
						...
						<view class="inputInfosList">
							<view class="flexA" style="margin: 28rpx 0;" v-for="(items,indexs) in item.formOptionsList" :key="indexs">
								<image src="/static/images/icon_chos@3x.png" mode="" class="inputInfosListImg"></image>
								{{items.text}}
								<text v-if="items.score" style="margin-left: 14rpx;">{{'('+items.score+'分)'}}</text>
							</view>
						</view>
					</view>
					<view class="editByutton" v-if="selectIndex==index&& id !==0">
		...
		<view class="addFormButton flexAC" @click="formInfo" v-if="id==0">
