<template>
	<view class="chat">
		...
		<scroll-view :refresher-enabled="enabledScroll" :scroll-into-view="toView" :refresher-threshold="30"
			...
			<view class="chatInfo">
				<view class="chatList">
					<view :class="index==0?'lastChatInfos chatInfos':'chatInfos'" v-for="(item,index) in chatList" :key="index" :id="'msg-'+index">
						<view class="time">
							{{item.createTime}}
						</view>
						<!--医生的信息-->
						<view class="docInfo flex" v-if="item.fromUid == docInfo.id">
							<view class="docInfos">
								...
								<!--表单信息-->
								<view class="content videoContent formContent" v-if="item.msgType=='FORM'" @click="toFormDetail(item,2)">
								...
								<view class="content videoContent" @click="voiceActive(item.url,index,item.id,item.fromUid)" v-if="item.msgType=='VIDEO'">
									...
									<view v-if="item.msgType=='VIDEO'&&index==playIndex&&isPlayVoice" class="playVoiceBox rightImg">
										<image src="/static/images/playVoice.png" class="voiceImg" v-if="!playImgone"></image>
							...
							<image :src="item.user.avatar?item.user.avatar:'/static/images/defaultAvatar.png'" alt="" class="avatorImg"></image>
						</view>
						<!--患者的信息-->
						<view class="patientInfo flex" v-if="item.fromUid !== docInfo.id">
							<image class="avatorImg"  @click="toPatientDetail(item)" :src="item.user.avatar&&item.user.avatar!='https://thirdwx.qlogo.cn/mmopen/vi_32/POgEwh4mIHO4nibH0KlMECNjjGxQUq24ZEaGT4poC6icRiccVGKSyXwibcPq4BWmiaIGuG1icwxaQX6grC9VemZoJ8rg/132'?item.user.avatar:'/static/images/user_default_icon.png'" alt="" ></image>
							<view class="patientInfos">
								<view class="name">{{item.fromUid==targetUid?item.user.patientName:item.user.nickname}}</view>
								<!--文字信息-->
								<view class="content requesContent" v-if="item.msgType=='MESSAGE_TEXT'">
									{{item.msg}}
								</view>
								...
								<view class="content videoContent" @click="voiceActive(item.url,index,item.id,item.fromUid)" v-if="item.msgType=='VIDEO'">
									...
									<view>{{item.videoDuration}}s″</view>
									<view v-if="item.msgType=='VIDEO'&&index==playIndex&&isPlayVoice" class="leftVoiceBox rightImg">
										<image src="/static/images/playVoice.png" class="voiceImg" v-if="!playImgone"></image>
										<image src="/static/images/playVoice.png" class="voiceImg"></image>
									</view>
								</view>
								<!--表单信息-->
								<view class="content videoContent formContent" v-if="item.msgType=='FORM'" @click="toFormDetail(item,1)">
									<image src="/static/images/icon_for@2x.png" mode="" class="formImg" ></image>
									<view class="formTitle">{{item.form? item.form.title:''}}</view>
								</view>
								<!--图片信息-->
								...
								<!-- 图文咨询 -->
								<view class="content requesContent" v-if="item.msgType=='PIC_CONSULTATION'" @click="information(item)">
									<view class="">
										咨询类型:{{item.msg}}
									</view>
									...
									<image v-if="item.patientOtherOrder.imageUrlList&&item.patientOtherOrder.imageUrlList.length>0" :src="item.patientOtherOrder.imageUrlList[0]" mode="" class="mesImg" ></image>
									<view class="">
		...
		<view class="inputButton " :style="showImgs?'padding: 30rpx 0 0 0;':'padding: 30rpx 0 ;'">
			<view class="flexAB" style="padding: 0 34rpx;">
				<image src="/static/images/btn_yuyin@2x.png" class="showImg leftOption" @click="showVoiceImg" v-if="inputText"></image>
				<image src="/static/images/btn_dazi@2x.png" class="showImg leftOption" @click="inputText=true" v-if="!inputText"></image>
				<view v-if="!inputText" class="innerPuters voiceInput" @longpress="startVoice" @touchend="stopVoice">按住 说话</view>
				<textarea v-if="inputText" v-model="innerValue" auto-height="true" @input="inputSendText"   cursor-spacing="30"
				:show-confirm-bar="false" type="text"  placeholder="请输入咨询内容" class="innerPuters"/>
				<!-- :disable-default-padding="true" -->
				<image src="/static/images/icon_add@2x1.png" mode="" class="showImg rightOption" @click="openMore" v-if="!inpuInfo"></image>
...
<script>
	...
	import {dateToTimestamp} from '@/static/utils/utils.js'
	export default {
		data() {
			return {
				formId:null,//推送的表单id
		...
		methods:{
			...
			getMessage(msg) {
				...
				if(dataMsg.msgType=='AUTH_RESULT'){
					...
					console.log(dataMsg.data,"kkkkkkkkkkkkkkkkkkkkkkk")
